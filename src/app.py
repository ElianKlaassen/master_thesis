...
app.layout = html.Div([
    ...
    dcc.Store(id='clicked-indices', data=clicked_indices),
...
def update_plot(query_idx, labeled_idx, current_children, relayout_data, reset_bool, iteration_count):
    ...
    return fig, reset_bool, df_bc.index.tolist()
...
@app.callback(
    ...
    State('clicked-indices', 'data'),
    prevent_initial_call='initial_duplicate'
)
def handle_labeling(click_data, reset_bool, labeled_idx, pcp_df, query_idx, current_children, check_values, clicked_indices):
    MT = np.zeros(len(data.columns))
    df_pcp = pd.DataFrame([MT], columns=data.columns)
    pcp = create_pcp(df_pcp)
